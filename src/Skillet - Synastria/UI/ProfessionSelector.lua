...
---@param self table Skillet addon instance
...
local function CreateProfessionSelector(self, parent)
    ...
    self.professionButtons = {} ---@type ProfessionButton[]

    for spellIndex, spellIdCollection in ipairs(professionSpellIds) do
        local spellId = nil ---@type number|nil
        ...
        if spellId then
            local name, _, icon = GetSpellInfo(spellId) ---@type string|nil, any, string|nil

            -- Skip if spell name or icon is missing
            if name and icon then
                -- Create button
                local button = CreateFrame('Button', 'SkilletProfessionButton' .. spellIndex, holder,
                    'SecureActionButtonTemplate') ---@type Frame
    ...
    local buttons = self.professionButtons ---@type ProfessionButton[]
    for i, prof in ipairs(buttons) do
...
local function UpdateProfessionButtons(self)
    ...
    for _, prof in ipairs(buttons) do
