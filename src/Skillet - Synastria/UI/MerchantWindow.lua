...
local merchant_inventory = {} ---@type table<string, boolean>

-- Get the name for the item in the specified merchant slot. Can
-- only be called when the merchant window is open
---@param slot number The merchant slot index
---@return string? name The item name at the slot, or nil if not found
...
---@param list ShoppingListItem[]|nil Array of items to check
---@return boolean hasRequired Whether the merchant sells any required items
local function does_merchant_sell_required_items(list)
	if not list then return false end
	---@cast list ShoppingListItem[]
	for i = 1, #list, 1 do
		local item = list[i] ---@type ShoppingListItem
	...
	for i = 1, count, 1 do
		...
		if itemname then
			...
			if numAvailable == -1 then
				...
				for j = 1, #list, 1 do
					local item = list[j] ---@type ShoppingListItem
				...
				if count > 0 then
					local name, texture, price, quantity, numAvailable, isUsable = GetMerchantItemInfo(i);
					local sName, sLink, iQuality, iLevel, iMinLevel, sType, sSubType, stackSize = GetItemInfo(
