...
---@param playername string|nil Optional player name filter (nil = all players)
---@param includeBank boolean Include bank items in availability calculation
---@param silent boolean If true, suppress chat output (for automatic exports)
---@return boolean success True if export succeeded
---@return number itemsAdded Count of items added
function Skillet:ExportShoppingListToResourceTracker(playername, includeBank, silent)
    if not self:IsResourceTrackerAvailable() then
        if not silent then
            self:Print(
                "|cFFFF0000ResourceTracker addon not found! Please install ResourceTracker to use this feature.|r")
        end
        return false, 0
    end
                ...
                if currentAmount < totalNeeded then
                    ...
                    if not canConvert then
                        ...
                        if recipe then
                            ...
                            local numCraftable = self.CraftCalc:CalculateRecipeCraftability(recipe, self.stitch,
                                includeBank, false, 0, false)
...
--- @param itemLink string: The item link or item ID
--- @param goalAmount number: How many we need (goal)
    ...
    for player, playerqueues in pairs(self:GetAllQueues()) do
        ...
        if queue and #queue > 0 then
            for i = 1, #queue do
                local item = queue[i]
                queueHash = queueHash ..
                    (item.profession or "") .. ":" .. (item.index or "") .. ":" .. (item.numcasts or "") .. "|"
            end
        end
    end
...
---@param recipe Recipe The recipe that was crafted
---@param numCrafted number How many times the recipe was crafted
---@return nil
    ...
    if addon == "Skillet - Synastria" then
        if Skillet:IsResourceTrackerAvailable() then
            ...
            DEFAULT_CHAT_FRAME:AddMessage(
                "|cFFFFAA00Use '/skillet exportrt' or the shopping list button to export items.|r")
        end
