...
local itemmeta = {
    __index = function(self,key)
        ...
        elseif key == "numcraftablewbank" then
            ...
            return num
        elseif key == "numcraftablewresbank" then
            -- Synastria: Calculate craftable with resource bank included
            local num = 1000
            for _,v in ipairs(self) do
                if v.vendor == false then
                    local max = math.floor(v.numwresbank/v.needed)*self.nummade
                    if max < num then
                        num = max
                    end
                end
...
local reagentmeta = {
    __index = function(self,key)
        ...
        elseif key == "numwresbank" then
            -- Synastria: Include resource bank in count
